              ...
              {examples.map((ex, i) => (
                <div key={i}
                  ...
                  <div className="p-5 space-y-4">
                    {/* 입력값 */}
                    <div>
                      <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">입력값</p>
                      <div className="grid grid-cols-2 gap-x-6 gap-y-1">
                        {ex.inputs.map((inp, j) => (
                          <div key={j} className="flex items-center justify-between text-sm">
                            <span className="text-slate-500">{inp.label}</span>
                            <span className="font-semibold text-slate-800 tabular-nums">{inp.value}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* 결과값 */}
                    <div>
                      <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">결과</p>
                          ...
                          <div key={j}
                            className={`flex items-center justify-between text-sm rounded-lg px-3 py-1.5
                              ${res.highlight
                                ...
                                : "text-slate-700"}`}>
                            <span className={res.highlight ? "" : "text-slate-500"}>{res.label}</span>
              ...
              {faq.map((item, i) => (
                <details key={i}
                  ...
                  <summary className="flex items-center justify-between px-5 py-4 cursor-pointer
                    ...
                    <span>Q. {item.q}</span>
                    <svg className="w-4 h-4 text-slate-400 transition-transform group-open:rotate-180 shrink-0 ml-3"
